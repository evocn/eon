...
*/
    ...
    // These are just the numbers which look good for this current heightmap. 
    float range_of_heightmap_values = 10.0;
    float y_offset = 2.0;

    float relative_height = (fragment_position.y + y_offset) / range_of_heightmap_values;
    float height = clamp(relative_height, 0, 1);

    FragColor.xyz = base_color * height;
